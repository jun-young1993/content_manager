...
function updateAddressInfo(addresses, port) {
    console.log('updateAddressInfo',addresses,port);
    ...
    if (previousIpPortString != newIpPortString) {
        ...
        console.log("=>(main.js:25) addresses, new IpPortString", addresses, newIpPortString);
        if (addresses.length > 0) {
            console.log("=>(main.js:27) /share/views/public/qr_codes/${addresses[0]}_${port}.png", `/share/views/public/qr_codes/${addresses[0]}_${port}.png`);
            theQRCodeElement.setAttribute("src", `/share/views/public/qr_codes/${addresses[0]}_${port}.png`)
        } else {
...
function getServerInfo(oldMD5 = null) {
    return new Promise((resolve, reject) => {
        const request = new XMLHttpRequest();
        const URL = (oldMD5 != null ? `/share/info?md5=${encodeURIComponent(oldMD5)}` : '/share/info');
...
function refreshInfoOnPage() {
    return getServerInfo(previousRootContentMD5).then((infoResult) => {
        ...
        console.log("(port != null && !isNaN(port))",(port != null && !isNaN(port)));
        console.log("(addresses != null && (addresses instanceof Array))",(addresses != null && (addresses instanceof Array)));
        if (
